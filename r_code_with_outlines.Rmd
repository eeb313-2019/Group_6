...
```{r}
pH.lm.sat.f<-lmer(pH~Season*siteID +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= TRUE)
pH.lm.sat<-lmer(pH~Season*siteID +(1|year) +(1|month), data= cleaned.final.ph, REML= TRUE)
pH.lm.sat.y<-lmer(pH~Season*siteID +(1|year), data= cleaned.final.ph, REML= TRUE)
pH.lm.sat.m<-lmer(pH~Season*siteID  +(1|month), data= cleaned.final.ph, REML= TRUE)

MuMIn::AICc(pH.lm.sat.m, pH.lm.sat.y, pH.lm.sat, pH.lm.sat.f)

```
# Linear Models - best fixed structure 
```{r}
pH.lm.sat.f<-lmer(pH~Season*siteID +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= FALSE)
pH.lm.sat.season<-lmer(pH~Season +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= FALSE)
pH.lm.sat.siteID<-lmer(pH~siteID +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= FALSE)
pH.lm.sat.no.int<-lmer(pH~Season +siteID +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= FALSE)
pH.lm.sat.ind<-lmer(pH~1 +(1|year) +(1|month) +(1|month:year), data= cleaned.final.ph, REML= FALSE)

MuMIn::AICc(pH.lm.sat.m, pH.lm.sat.season, pH.lm.sat.siteID, pH.lm.sat.no.int, pH.lm.sat.ind)

# best structure is pH.lm.sat.siteID
```
